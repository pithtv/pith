...
function fromNpt(time) {
    return time.match(/([0-9]+):([0-9]{2}):([0-9]{2})/).slice(1).reduce(function(a,b) { return a*60 + parseInt(b); },0);
}
...
var vidStreamer = function (req, res) {
    ...
    settings.getFile(info.path, function(filePath) {
        ff.ffprobe(filePath, function(err, metadata) {
            ...
            downloadHeader(res, info, metadata);
...
var downloadHeader = function (res, info, metadata) {
	...
	if(metadata) {
        header["X-AvailableSeekRange"] = "1 npt=0-" + metadata.format.duration;
