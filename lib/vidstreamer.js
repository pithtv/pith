...
var keyframes = require('retrieve-keyframes');
...
function toNpt(duration) {
    var d = Math.ceil(duration);
    return sprintf('%d:%02d:%02d', d / 3600, d % 3600 / 60, d % 60);
}

function fromNpt(time) {
    return time.match(/([0-9]+):([0-9]{2}):([0-9]{2})/).slice(1).reduce(function (a, b) {
...
var vidStreamer = function (req, res) {
    ...
    var transcode = req.param("transcode");
    var generatePlaylist = req.param("playlist");
    var offset = req.param("start");
        ...
        if (generatePlaylist == "m3u8") {
            ff.ffprobe(filePath, (err, metadata) => {
                ...
                if(formats.indexOf('mp4') > -1) {
                    keyframes.getForMp4(filePath, (err, frames) => {
                        writePlayList(frames);
                    });
                } else if(formats.indexOf('matroska') > -1) {
                    keyframes.getForMkv(filePath, (err, frames) => {
                        writePlayList(frames);
                    });
                } else {
                    writePlayList();
...
var downloadHeader = function (res, info, metadata) {
    var code = 200;
    var header;
